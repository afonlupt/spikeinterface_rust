...
mod rust_peak_detection_locally_exclusive_sam3;
...
pub fn peak_detection(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(rust_peak_detection_locally_exclusive_sam3::detect_peaks_rust_locally_exclusive_on_chunk, m)?)?;
    Ok(())
}
